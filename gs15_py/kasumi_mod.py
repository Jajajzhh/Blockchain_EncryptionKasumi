...
#The fonction RC4 to generate a list of 256 = 2^8 as the S-box for our modified FI fonction
def KSA(key):
...
class Kasumi:
    ...
    def __init__(self):
        ...
        self.key_KI3 = [None] * 9
        rc4_key = "rc4gs15"
        self.s8_box = KSA(rc4_key)

    #Generation de cle a base de cle master
    ...
    #Function FI division 7+9 passer par s-box s7 et s9 => 3 rounds Fiestel esque + Mod GS15 S box generer par RC4 + division 8+8 Euclidien division
    ...
    #Function Fo utilise FI passant par 3 fois Fiestel esque
    def fun_FO(self, input, round_i):
        ...
        out_right = self.fun_FI_mod(in_left ^ self.key_KO1[round_i], 
                                          self.key_KI1[round_i]) ^ in_right

        in_left   = out_right # use in_* as temp variables
        in_right  = self.fun_FI_mod(out_left ^ self.key_KO2[round_i],
                                           self.key_KI2[round_i]) ^ out_right

        out_left  = in_right
        out_right = self.fun_FI_mod(in_left ^ self.key_KO3[round_i], 
    ...
    #Function fl plutot simple avec ROL 1 fois passer par cle Mod GS15 une inversion Gallois avec polynome 16
    def fun_FL(self, input, round_i):
        ...
        assert _bitlen(out_left)  <= 16
        assert _bitlen(out_right) <= 16
        out_left = inv.gf_inv(out_left)
        out_right = inv.gf_inv(out_right)
        return (out_left << 16) | out_right

    #Function F of the encryption methode: It varies between FL and FO
    ...
    #Encrytion KASUMI= encryption_1r X 8
    ...
    def fun_FI_mod(self, input, round_key):
        # assert _bitlen(input)  <= 16
        #Left = 8bit right = 8bit
        left  = input >> 8
        right = input & 0b11111111
        left_s = self.s8_box[left]
        right_s = self.s8_box[right]
        fi_out = (round_key >> 2) ^ (left_s | right_s)
        # assert _bitlen(left)  <= 7
        # assert _bitlen(right) <= 9
        return fi_out
    
    def fun_FL_mod(self, input, round_i):
        # assert _bitlen(input)  <= 32
        # assert round_i >= 1 and round_i <= 8
        ...
        # assert _bitlen(out_left)  <= 16
        # assert _bitlen(out_right) <= 16
        #Transfer with inversion mod = 0x18f57 => x^16 + x^15 + x^11 + x^10 + x^9 + x^8 + x^6 + x^4 + x^2 + x^1 + 1 Polynome irreductible
        out_left = inv.gf_inv(out_left)
        out_right = inv.gf_inv(out_right)
        return (out_left << 16) | out_right
