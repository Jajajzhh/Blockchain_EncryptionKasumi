...
def padding(msg):
    #First, we pad the message to a size containing only 64 bits blocks = 8*8 bytes
    msgLen = bitlen(msg)%(2**64)
    msg = msg + b'\x80'
    zeroPad = (64 - (msgLen+8)%64)%64
    zeroPad //= 8
    msg = msg + b'\x00'*zeroPad + msgLen.to_bytes(8,byteorder='little')
    msgLen = bitlen(msg)
    iterations = msgLen//64
    return msg, iterations


def sponge_absorb(msg_64, iterations):
    #Then every 64 bit of the message get plused with 64*0, xor data(r+c) then go through md5 to get a new data(r+c) until msg_64 run out
    state = 0
    for i in range (0, iterations):
        #rateblocks = blockDivide(msg_64,iterations)
        #rate = rateblocks[i]
        rate = msg_64[i*8:(i+1)*8]
        rate_int = int.from_bytes(rate,byteorder='little')
        rate_int = rate_int<<64
        state = state ^ rate_int
        #16*8=128 bits 16 bytes
        state_bytes = state.to_bytes(16,byteorder='little')
        state = md5sum(state_bytes)
    return state

def sponge_squeeze(state,i=2):
    #Having a state data that finished absorbing, we run the state through md5 for 2 times to get 128 bits, or 4 times to get 256 bits.
    #state = md5sum(state)
    out = 0
    for i in range (i):
        state_bytes = state.to_bytes(16,byteorder='little')
        ...
        out = (out<<64|rate)
    return out
#redefined MD5 fonction takes in string and output by 128 bits
def hash_sponge(data):
    if type(data) != bytes:
        data = bytes(data,'utf-8')
    msg,iteration = padding(data)
    absorb = sponge_absorb(msg,iteration)
    squeeze = sponge_squeeze(absorb)
    print(hex(squeeze))
    return squeeze

if __name__ == "__main__":
    data = 'hello world_fuck gs1'
    sq = hash_sponge(data)
    #arr = bytes(data, 'utf-8')
    #msg,iteration = padding(arr)
    #msg = md5sum(arr)
    #print(len(hex(msg)))
    #rate = msg[1*8:(1+1)*8]
    #rate_int = int.from_bytes(rate,byteorder='little')
